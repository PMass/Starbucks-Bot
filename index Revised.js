...
client.on('message', message => {
	...
	const guild = client.guilds.cache.get('294226224446701568') // get the guild object
	const member = guild.member(user) // Server member info
	if (message.author.id != '794000856516395009'){ // If not the bot
		if (message.channel.id == '795464681912795156' && message.content == '$verify') { // If in the proper channel and proper message
			...
			console.log('Gave Temp verified role to ' + user.tag) // Log in the console that we giving them the temp role to them
 			member.roles.add('795463783149207572'); // Give them the temp verified role
  			const channelID = '795464295822917672' // temp channel ID
  			const channelFull = guild.channels.cache.get(channelID)  // Full information for temp channel		
 			sendMessage(channelFull,`${tag} you have 25 seconds to paste a link to your verification here! It will be remove after you post it`, 15) // Send a message in the temp channel
			sleep(60000).then(() => { // wait for 30 seconds
				console.log('Removed temp verified role for ' + user.tag)
 				member.roles.remove('795463783149207572'); //Remove the temp role if they have waited longer than 30 seconds
			});
		}
		if (message.channel.id == '795464295822917672') { //If in the temp channel and not the bot
			...
			member.roles.remove('795463783149207572'); //Remove their role
		}
		if (message.channel.id == '795464681912795156' && message.content != '$verify' ) { // If in the proper channel and proper message
...
client.on('messageReactionAdd', (messageReaction, user) => { //when we react
    let message = messageReaction.message, emoji = messageReaction.emoji; //Log this shit
 	...
 	if (userid != '794000856516395009' && message.channel.id == '786423882608410664') { // If not the bot
...
async function saveVerification(userid,verificationURL) { 
	...
	const guild = client.guilds.cache.get('294226224446701568') // get the guild object
  	const channelFull = guild.channels.cache.get('786423882608410664') 	//Channel for sending verification log message
	...
	var intakeIndex = verifiedMessageArray.findIndex(intakePosition); //find this element in the verified messages
 	...
 	assignVerified(userid,status) // run the function to assign roles
    	...
    	if(googleError == '401' ||googleError == '429' ){ //If for some reason google is rate limiting us
 			const guild = client.guilds.cache.get('294226224446701568')
  			const channelID = '795464681912795156'
...
function assignVerified(userid,status){
 	...
 	const channelID = '795464681912795156' //Main Verification channel ID
